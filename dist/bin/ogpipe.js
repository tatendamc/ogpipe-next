...
function resolveTemplateHtml(template, configDir) {
  ...
  if (template.file) {
    const filePath = (0, import_path.resolve)(configDir, template.file);
    try {
...
function watchTemplateFiles(config, configDir, onChange) {
  ...
  for (const template of Object.values(config.templates)) {
    if (template.file) {
      const fullPath = (0, import_path3.resolve)(configDir, template.file);
  ...
  for (const filePath of filesToWatch) {
    (0, import_fs3.watch)(filePath, { persistent: false }, (eventType) => {
      if (eventType === "change") {
...
var import_http, import_fs3, import_path3, DEFAULT_PORT;
...
async function generateOGImages(options) {
  ...
  const manifestPath = (0, import_path2.join)(projectDir, ".next", "prerender-manifest.json");
  ...
  const manifest = JSON.parse(
    (0, import_fs2.readFileSync)(manifestPath, "utf-8")
  );
  ...
  const outDir = (0, import_path2.resolve)(projectDir, config.outDir || "public/og");
  ...
  (0, import_fs2.writeFileSync)(
    (0, import_path2.join)(outDir, "og-manifest.json"),
...
async function generateSingleImage(options) {
  ...
  const outputPath = (0, import_path2.join)(outDir, filename);
  ...
  if (imageBuffer) {
    (0, import_fs2.mkdirSync)((0, import_path2.dirname)(outputPath), { recursive: true });
    (0, import_fs2.writeFileSync)(outputPath, imageBuffer);
  }
  return {
    route,
...
async function runGenerate() {
  ...
  if (!(0, import_fs4.existsSync)((0, import_path4.resolve)(projectDir, ".next"))) {
...
async function loadConfig(projectDir) {
  const configPaths = [
    (0, import_path4.resolve)(projectDir, "ogpipe.config.ts"),
    (0, import_path4.resolve)(projectDir, "ogpipe.config.js"),
    (0, import_path4.resolve)(projectDir, "ogpipe.config.mjs")
  ];
  for (const configPath of configPaths) {
    if ((0, import_fs4.existsSync)(configPath)) {
      try {
        ...
        try {
          const module2 = await import(`file://${configPath}`);
          return module2.default || module2;
        } catch {
...
//# sourceMappingURL=ogpipe.js.map